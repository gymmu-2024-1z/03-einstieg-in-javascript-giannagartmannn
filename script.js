...
export function bubbleSort(args) {
  // Erzeugt eine Kopie des Eingabearrays
  const text = args
  //Die Eingabe wird zu einer Reihe (Array) von Zeichen umgewandelt
  const list = text.split("")
  for (let i = 0; i < list.length - 1; i++) {
    const currentElement = list[i] //das aktuelle Element, welches mit dem nächsten verglichen wird
    const nextElement = list[i + 1] //das nächste Element im Array
    if (currentElement.charCodeAt(0) > nextElement.charCodeAt(0)) {
      // Reihenfolge stimmt nicht, Elemente müssen getauscht werden
      const tmp = list[i + 1]
      list[i + 1] = list[i]
      list[i] = tmp
      i = -1 // Starte von vorne, damit alles sortiert wird
    }
  }
...
export function countingSort(args) {
  ...
  for (let i = 0; i < lookup.length; i++) {
    //schaue die Häufigkeit der ASCII-Wertes nach, um so zu sortieren
    const value = lookup[i] //hole den ASCII-Wert des Zeichens
    for (let j = 0; j < value; j++) {
      const character = String.fromCharCode(i) //der ASCII-Wert wird zurück in ein Zeichen umgewandelt
      result.push(character)
    }
  }
  return result.join("") //die sortierten Zeichen werden zu einer Liste, die uns angezeigt wird
}
